alter table public.app_records
  ...
  add column if not exists temp_f_min numeric,
  add column if not exists temp_f_max numeric,
  add column if not exists wind_speed_mph_min numeric,
  add column if not exists wind_speed_mph_max numeric;

create table if not exists public.app_record_fields (
...
create index if not exists idx_app_record_fields_record_id on public.app_record_fields (app_record_id);

alter table public.app_record_fields enable row level security;

create policy "Authenticated users can manage app_record_fields"
  on public.app_record_fields for all to authenticated using (true) with check (true);

-- Signatures change (new p_fields arg), so drop old versions first.
drop function if exists public.create_app_record_with_children(jsonb, jsonb, jsonb);
drop function if exists public.update_app_record_with_children(uuid, jsonb, jsonb, jsonb);
...
begin
  ...
  insert into public.app_records (
    job_date, applicator_name, customer_name, customer_id, site_address, job_site_id,
    location_lat, location_lng,
    temp_f, temp_f_min, temp_f_max,
    wind_speed_mph, wind_speed_mph_min, wind_speed_mph_max, wind_direction, sky_condition,
    target_vegetation, target_veg_other, app_method, app_type,
    start_time, end_time,
    total_gallons, gallons_per_acre, acres_treated, tank_mix_record,
    equipment_notes, equipment_id, truck_id, nozzle_type,
  ...
  values (
    ...
    nullif(p_record->>'customer_id', '')::uuid,
    ...
    nullif(p_record->>'temp_f_min', '')::numeric,
    nullif(p_record->>'temp_f_max', '')::numeric,
    nullif(p_record->>'wind_speed_mph', '')::numeric,
    nullif(p_record->>'wind_speed_mph_min', '')::numeric,
    nullif(p_record->>'wind_speed_mph_max', '')::numeric,
  ...
  insert into public.app_record_fields (
    app_record_id, field_id, field_name_snapshot, location_lat, location_lng, sort_order
  )
  select
    v_record_id,
    nullif(f->>'field_id', '')::uuid,
    nullif(f->>'field_name_snapshot', ''),
    nullif(f->>'location_lat', '')::numeric,
    nullif(f->>'location_lng', '')::numeric,
    coalesce((f->>'sort_order')::int, ordinality - 1)
  from jsonb_array_elements(coalesce(p_fields, '[]'::jsonb))
    with ordinality as entry(f, ordinality);
...
begin
  ...
  update public.app_records set
    ...
    customer_id = nullif(p_record->>'customer_id', '')::uuid,
    ...
    temp_f_min = nullif(p_record->>'temp_f_min', '')::numeric,
    temp_f_max = nullif(p_record->>'temp_f_max', '')::numeric,
    wind_speed_mph = nullif(p_record->>'wind_speed_mph', '')::numeric,
    wind_speed_mph_min = nullif(p_record->>'wind_speed_mph_min', '')::numeric,
    wind_speed_mph_max = nullif(p_record->>'wind_speed_mph_max', '')::numeric,
  ...
  delete from public.app_record_fields where app_record_id = p_record_id;
...
grant execute on function public.create_app_record_with_children(jsonb, jsonb, jsonb, jsonb) to authenticated;
grant execute on function public.update_app_record_with_children(uuid, jsonb, jsonb, jsonb, jsonb) to authenticated;
